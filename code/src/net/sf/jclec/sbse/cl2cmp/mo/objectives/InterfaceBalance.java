...
/**
 * Interface Balance metric. This metric
 * evaluates the balance between the number
 * of components and the number of interfaces. 
 * Inspired by the Component Balance
 * metric defined in "Quantifying the analyzability
 ...
 * @see Cl2CmpObjective
 * */
public class InterfaceBalance extends Cl2CmpObjective {
	...
	/** Serial ID */
	private static final long serialVersionUID = -6650560346125526049L;
	...
	protected IFitness compute(Cl2CmpMoIndividual ind) {
		...
		int opt = (int)((float)getMaxInterfaces()/2.0);
		int n = ind.getNumberOfComponents();
		int min = getMinComponents()-1;
		int max = getMaxInterfaces()+1;
		...
		// Component interfaces uniformity
		double [] sizes = new double[n];
		for(int i=0; i<n; i++){
			sizes[i] = ind.getNumberOfProvided(i)+ind.getNumberOfRequired(i);
		}
